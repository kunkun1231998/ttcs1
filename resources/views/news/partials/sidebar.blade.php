@php
	$list_block_keys = ['featured_post','fanpage','tags','followus','videos','email','adv'];
	$order_list = array_map(function($value){
		return [
			'order' => config('global.sidebar_order_' . $value),
			'block' => $value
		];
	},$list_block_keys);
	$ordered_list = collect($order_list)->sortBy('order')->keyBy('order')->transform(function ($value){
			return $value['block'];
	})->all();
@endphp
<aside id="sidebar" class="four column pull-right">
	<ul class="no-bullet">
		@foreach ($ordered_list as $key => $block)
				@if($list_block_keys[0] === $block)
				...
				@endif
				@if($list_block_keys[1] === $block)
					<li class="widget widget_facebook_box clearfix">
						<h3 class="widget-title">Fanpage Facebook </h3>
						<iframe
							src="http://www.facebook.com/plugins/likebox.php?href=http://facebook.com/laravelvn	&amp;width=285&amp;colorscheme=light&amp;show_faces=true&amp;border_color&amp;stream=false&amp;header=false&amp;height=300"
							scrolling="no" frameborder="0" allowtransparency="true"></iframe>
					</li>
				@endif
				@if($list_block_keys[2] === $block)
				...
				@endif
				@if($list_block_keys[3] === $block)
					<li class="widget widget_social_media clearfix">
						<h3 class="widget-title">Follow us</h3>
						<ul class="no-bullet">
							<li class="twitter">
								<div class="btn">
									<a href="https://twitter.com/nextWPthemes" class="twitter-follow-button" data-show-count="false"
							...
							<li class="facebook">
								<div class="btn">
									...
									<div class="fb-like" data-href="http://www.nextwpthemes.com/" data-send="false"
										data-layout="button_count" data-width="450" data-show-faces="true"></div>
								</div>
							</li>
							<li class="pinterest">
								<div class="btn">
									<a href="http://pinterest.com/pin/create/button/?url=http%3A%2F%2Fthemeforest.net%2Fitem%2Fnext-magazine-responsive-magazine-template%2F2576082&amp;media=http%3A%2F%2F1.s3.envato.com%2Ffiles%2F29793891%2Fscreenshots%2F00_preview.__large_preview.jpg&amp;description=Next+Magazine+-+Responsive+Magazine+Template"
										class="pin-it-button" count-layout="horizontal"><img alt="Pin It" border="0"
				...
				<li class="widget widget_video clearfix">
					...
					<div class="flex-video widescreen">
						...
						@if(!empty($post_video[0]))
							<video src="{{ $post_video[0]->feture }}" alt="{{ $post_video[0]->title }}"
								style="width: 100%" controls></video>
						@endif
					</div>
				</li>
				@endif
				@if($list_block_keys[5] === $block)
